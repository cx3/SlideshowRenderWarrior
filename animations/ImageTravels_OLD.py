...
class ImageTravel(NextFrame):
    ...
    def set_params(self, image: image_types, frames_count: int, frame_resolution: list or tuple = (1920, 1080)):
        self.image = image
        self.frames_count = frames_count
        self.frame_resolution = frame_resolution
    ...
    def decrease_position(self):
        self.crop_params.decrease_position()


class _ExampleTravel(ImageTravel):
...
def get_random_travellers_circlelist(travels_count=-1, shuffle_list=True, debug=False) -> utils.CircleList:
    if debug:
        return utils.CircleList(
            list_or_tuple=[_ExampleTravel] * 100,
        )

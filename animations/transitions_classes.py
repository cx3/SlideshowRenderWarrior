...
class Transition(object):
    def __init__(
        ...
        '''if image1 is None and image2 is None:
            ...
            if not isinstance(kwargs['parent_timeline'], RenderControllers.TimelineModel):
                raise AttributeError(err)'''
        ...
        if 'frame_resolution' not in kwargs:
            if self.im1 is None:
                tl = kwargs.get('parent_timeline', None)
                if tl is not None:
    ...
    def set_effects(self, kwargs: dict):
        ...
        for item in ('preprocess_first', 'preprocess_second', 'postprocess_first', 'postprocess_second'):
            ...
            print(f'@@ Transition.set_effects item={item} of type={type(value)}')
    ...
    def next_frame(self, **params) -> dict:
        ...
        print('\n\tTRANSITION BASE CLASS next_frame starts HERE\n')
        ...
        print('\n\tTRANSITION BASE CLASS next_frame ends HERE\n')
        return params
    ...
    def _next_frame(self, frame_index: int, im1: image_types, im2: image_types, **params) -> dict:
        ...
        im1 = PIL.Image.blend(im1, im2, frame_index * step)  # transition blending here
        im1, im2 = self._postprocess(im1, im2, frame_index=frame_index)
        params['to_save'] = im1

        return params

    def render_frames(self, image1, image2) -> list:
        old_pos = int(self.frame_index)
        result = []
        for i in range(self.frames_count):
            result.append(self.next_frame(im1=image1, im2=image2, frame_index=i)['to_save'])
        self.frame_index = old_pos
        return result
...
class BlendingTransition(Transition):
    ...
    def _next_frame(self, frame_index: int, im1: image_types, im2: image_types, **params) -> dict:
        ...
        im1 = utils.verify_alpha_channel(utils.convert_image_type(im1, np.ndarray))
        im2 = utils.verify_alpha_channel(utils.convert_image_type(im2, np.ndarray))
        ...
        ts = blend_images_effect(
            im2, im1, frame_index/self.frames_count, 1 - frame_index/self.frames_count,
